...
template <std::integral T>
unsigned bit<T>::size(){return len;}

template <std::integral T>
bool bit<T>::operator[](size_t idx) const{return (value>>idx)&static_cast<T>(1);}
    
template <std::integral T>
bit_proxy<T> bit<T>::operator[](size_t idx) {return bit_proxy<T>(value, idx);}
    
template <std::integral T>
bit<T> bit<T>::operator&(const bit& other){return bit(value&other.value);}
...
template <std::integral T>
bit<T> bit<T>::operator~(const bit& other){return bit(~value); }
